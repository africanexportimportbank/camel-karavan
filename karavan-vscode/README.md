...
* Run using CLI
    ```shell
    jbang -Dcamel.jbang.version=4.18.2 camel@apache/camel run $INTEGRATION.yaml --max-messages=10 --logging-level=info
    ```
...
* Export using CLI
    ```shell
    jbang -Dcamel.jbang.version=4.18.2 camel@apache/camel export --directory=export
    ```
